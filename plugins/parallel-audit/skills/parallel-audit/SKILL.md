...
4. **Exclusions** — pre-load defaults from `references/<target_type>-specifics.md`. Present as multi-select for deselection, accept free-text additions for skill-specific intentional design. While reading the target-specifics document for exclusion defaults, **also Read `references/shared-blind-spots.md`** and assemble `known_fp_patterns` as the union of the target-specifics "Common shared-blind-spot patterns" section + the shared-blind-spots entries. Store `known_fp_patterns` as a setup-time artifact passed unchanged to every Phase 6.5 dispatch (per the Verification subagents table input column). This is separate from the user-edited `exclusion_list` and not presented for user editing — the entries are pre-validated FP patterns, not exclusions
