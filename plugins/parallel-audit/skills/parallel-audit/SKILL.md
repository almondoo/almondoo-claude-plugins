...
3. **Confirm `N` / `threshold` / `max_iterations`** — defaults `3 / 2 / 3`. Offer an "opt into deep audit (N=5 or N=9)" option for cases where the user wants stronger convergence; show the per-iteration cost estimate for each tier. **Validate the chosen values before proceeding** (all three must be positive integers; reject non-integer / negative / non-numeric values with a stated re-ask):
   ...
   - **Re-ask cap**: if the user enters an invalid combination, re-ask up to **3 times total** (initial attempt is attempt #1; re-asks are attempts #2 and #3). After the 3rd attempt fails, present a final AskUserQuestion with three options: (a) "Use defaults `3/2/3` and proceed" — explicit user-chosen defaults, not silent coercion; (b) "Abort this audit"; (c) "Try once more" — gives one more attempt, then if that also fails, abort. The abort path tells the user what they tried last, what constraint failed, and how to re-invoke with valid parameters (a single-line summary), so they can recover without restarting from Phase 1 blind.
