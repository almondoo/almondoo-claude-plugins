...
| `N` | 3 | Number of parallel auditor subagents per iteration. Must be ≥ 2 (single-instance results are not convergence — see Phase 2 input validation). Opt-in to 5 or 9 for deeper convergence |
| `threshold` | 2 | Minimum instances that must flag an issue for it to be considered reproducible. Must satisfy `2 ≤ threshold ≤ N` (see Phase 2 input validation); the percentage agreement implied by `threshold/N` varies by tier — see Default calibration below |
...
3. **Confirm `N` / `threshold` / `max_iterations`** — defaults `3 / 2 / 3`. Offer an "opt into deep audit (N=5 or N=9)" option for cases where the user wants stronger convergence; show the per-iteration cost estimate for each tier. **Validate the chosen values before proceeding**: reject `N < 2` (single-instance results are noise, not convergence — the entire workflow premise depends on multiple independent reads) and reject `threshold > N` or `threshold < 2` (a threshold exceeding `N` makes `(N − threshold + 1)` ≤ 0 and the Phase 12 practical-convergence condition vacuously true on iteration 1 → silent instant stop with no audit; a threshold below 2 collapses to single-instance signal). If the user enters an invalid combination, re-ask with the constraint stated; do not silently coerce to defaults.
...
**Pre-check for overlapping fix line ranges.** Before issuing the first Edit, scan all approved-this-iteration fixes and group any whose `line_range` overlaps or sits within ±2 lines of another. Phase 9 (`fix-safety-checker`) judges each fix in isolation and does not detect inter-fix overlap; if two SAFE fixes both rewrite the same span, the second Edit can fail (the `before` text no longer matches after the first Edit lands) or — worse — land in a wrong location and silently corrupt the file. Handle each overlap group as follows: (a) apply the first fix in the group via Edit, (b) re-Read the affected line range, (c) re-evaluate the remaining fixes in the group against the new file content (the second fix's `before` text and its safety-check assumptions may now be stale — if any has been invalidated, surface the conflict to the user via AskUserQuestion with options Skip / Re-draft / Apply-as-is, and skip the remaining fixes in that group rather than guessing). Non-overlapping fixes can be applied in any order without re-Read between them.
...
| `Glob` | Phase 7 sibling-skill discovery (`target_type == "skill-md"` only) |
...
- **In-session triggering recall is unmeasured.** The only triggering measurement we have runs through the `skill-creator/run_loop.py` (`claude -p`) backend, where the skill recorded **recall = 0% on the 4 should-trigger evals** (eval ids 1–4). The `claude -p` triggering path is documented as different from the in-session triggering path the user actually experiences, but no in-session re-measurement has been performed. Practically: if you do not invoke the skill explicitly (e.g., via the `/parallel-audit:parallel-audit` command or by naming the skill in your prompt), the executor **may silently not trigger it** even on a clearly in-scope request. We recommend invoking it explicitly until the in-session recall is benchmarked.
- **End-to-end behavioral benchmarks do not exist for should-trigger evals.** Evals 1–4 in `evals/evals.json` are verified via trace review (reading SKILL.md prose and judging assertion clarity), not via end-to-end execution, because Phase 1 / 2 / 3 / 10 / 11 `AskUserQuestion` calls block subagent runs. The "should-not-trigger" evals (5–7) are end-to-end benchmarked. So the workflow's *correctness against the spec* is verified for negative cases only; for positive cases we have spec-coherence, not behavior.
