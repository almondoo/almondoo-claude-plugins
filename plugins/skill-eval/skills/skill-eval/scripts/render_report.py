...
def render(static: dict, benchmark: Optional[dict], target: str) -> str:
    ...
    lines += [
        "## Top issues to fix",
        "",
        "<derivation guide: references/proposal-derivation.md>",
        "<for each fix, capture: rank / category (instructions|tools|examples|error_handling|structure|references) / priority (high=outcome-changing / medium / low) / source (static|differentiating|time_token|dogfooding|assertion_rework) / rationale / estimated_effort / verifiability>",
        "",
        "<4 sources to scan, in order:>",
        "<  A. static FAIL/warn rows (above) — list any passed=false items by severity>",
        "<  B. differentiating assertions (above) — flip each: with-only-pass → contract to codify; without-only-pass → mis-steering to remove>",
        "<  C. time / token / variance anomalies — time delta ≥ 2× or tokens delta ≥ +50% → body is bloated; variance > mean × 0.3 (n ≥ 3) → flaky instructions>",
        "<  D. dogfooding gap — writing quality / placeholder resolution / translation ambiguity / internal consistency / script↔doc drift (not caught by static_check)>",
        "",
        "<cap: at most 3 items. Remainder → LEARNINGS.md open follow-ups section. Reject items that overfit to a single eval prompt.>",
        "",
        "1. ",
        "2. ",
        "3. ",
