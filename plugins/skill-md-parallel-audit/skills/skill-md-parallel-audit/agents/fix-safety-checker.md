...
The audit workflow's Phase 5 generates fix proposals based on the cited issue and surrounding context. But CLAUDE.md files are dense with cross-references: a phrase changed in one bullet may be cited verbatim in another section, an example removed from one list may be the only place a concept is grounded, and a "minor" rewording can flip the meaning of a downstream rule that depends on the exact wording.

This agent runs after Phase 5 drafts a fix and before the main thread shows it to the user, so that the user only sees fixes that have already been checked for cross-section safety.
...
3. **`proposed_fix`** — the before/after diff:
   ...
   classification: KEEP | SIMPLIFY | REMOVE  # from Phase 4.6
   ```
4. **`section_purposes`** — map from each section heading to its 1-line purpose, established in Phase 1.5 with user confirmation. Use this as the **authoritative baseline** for the `intent_preserved` check — do not re-derive section intent from the file content (that would be circular reasoning that re-reads the section being changed).
...
- **Do not draft a replacement fix.** Phase 5 owns fix drafting. You can suggest mitigations but do not write new before/after diffs.
